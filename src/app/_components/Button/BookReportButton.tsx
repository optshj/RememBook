...
export default function BookReportButton({ isbn13, className = "", large = false }: BookReportButtonProps) {
    ...
    return (
        <Link href={`/information?isbn13=${isbn13}`} scroll={false}>
            <button
                className={`${large ? "relative h-10 w-40" : "absolute bottom-2 right-2 px-2 py-1 text-sm"} flex items-center justify-center gap-0.5 overflow-hidden rounded-lg bg-button-black font-semibold text-white ${className}`}
                ref={contentRef}>
                <HiPencil />
                {ripples}
                {"기록하기"}
