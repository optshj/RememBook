    ...
    return (
        <div className={`group relative ${className}`}>
            ...
            <div className="absolute right-1 top-1 z-50 rounded-full" tabIndex={0} onClick={() => setIsOpen(prev => !prev)} onBlur={() => setIsOpen(false)}>
                ...
                <ul
                    className={`absolute right-0 top-full z-50 mt-2 w-52 rounded-lg bg-white font-semibold shadow-lg transition-all duration-300 ${isOpen ? "visible opacity-100" : "invisible opacity-0"}`}>
                    <li>
                        <Link
